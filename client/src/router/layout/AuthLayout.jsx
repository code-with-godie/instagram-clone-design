  ...
  return (
    ...
    <Container>
      <Header>
        ...
        <Description>Sign up to see photos and videos from you friends.</Description>
        <Button> <Facebook/> Log in with facebook</Button>
        <Divider>
          <Line/>
    ...
    <Footer>
      <FooterDescription>{`${login ? 'dont have an account?' : 'Have an account?'}`}   <Link to={`${login ? '/register' : '/login'}`} className='link' >{`${login ? 'sign up' : 'sign in'}`}</Link></FooterDescription>
    </Footer>
    <LinkWrapper>
    <LinksContainer>
    ...
    <Link className='link' >contact uploading and non users</Link>
    <Link className='link' >meta verified</Link>
    </LinksContainer>
    {/* <CopyrightContainer>
      <LanguageContainer>
